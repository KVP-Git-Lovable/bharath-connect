-- Make GPS-based Travel Allowance use the per-km rate that was effective on
-- each day (from ta_rate_history), instead of applying whichever rate is
-- currently configured to every day in the queried range.
--
-- Previously ta_rate_history only ever fed a single "current" value back
-- into expense_master_config.ta_per_km_rate (see TaRateHistory.tsx), and
-- get_monthly_expense_summary applied that one value to the whole month.
-- So the UI's own claim -- "changing the rate never re-prices past
-- records" -- was not actually true for the org-wide default rate.
--
-- Per-user/group/team TA overrides are unaffected: they still apply as a
-- single current value, since no history exists for overrides. Only the
-- org-wide default per-km rate becomes date-effective.
    ...
    SELECT d.d,
           ...
           CASE
             WHEN v_ta_type='from_gps' THEN
               ...
               (CASE WHEN v_ta_override_applied THEN v_ta_rate ELSE COALESCE(rh.rate, v_ta_rate) END)
