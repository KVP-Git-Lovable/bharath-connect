-- TA on/off switches:
-- * expense_master_config.ta_default_enabled: the "Everyone" row. Off = people not covered by another row get no TA.
-- * expense_groups.is_active: each custom TA row. Off = the row is ignored.

ALTER TABLE public.expense_master_config
  ADD COLUMN IF NOT EXISTS ta_default_enabled boolean NOT NULL DEFAULT true;
...
BEGIN
  ...
  IF v_cfg.id IS NOT NULL THEN
    ...
    v_ta_default_on := COALESCE(v_cfg.ta_default_enabled, true);
  END IF;
  -- Everyone row switched off: people not covered by a user/team/role row get no TA.
  IF NOT v_ta_default_on THEN
    v_ta_amount := 0;
    v_ta_rate := 0;
  END IF;
    ...
    ELSE
      ...
      IF NOT v_ta_override_applied THEN
        SELECT g.id INTO v_role_group_id FROM public.expense_groups g
          ...
          WHERE usp.user_id = _user_id AND g.is_active
          ORDER BY g.created_at LIMIT 1;
  ...
  per_day AS (
    SELECT d.d,
           ...
           -- Vehicle picked that day: employee's vehicle exception -> vehicle rate/price -> person's own TA.
           -- No vehicle picked: person's own TA (user/group/team/role override, else company default).
           CASE
               ...
               CASE
                 WHEN vs.vehicle_type_id IS NOT NULL THEN
                   COALESCE(
                     ...
                     CASE WHEN v_ta_override_applied OR v_role_rate_applied THEN v_ta_rate ELSE (CASE WHEN v_ta_default_on THEN COALESCE(rh.rate, v_ta_rate) ELSE 0 END) END)
                 WHEN v_ta_override_applied OR v_role_rate_applied THEN v_ta_rate
                 ELSE (CASE WHEN v_ta_default_on THEN COALESCE(rh.rate, v_ta_rate) ELSE 0 END)
               END
