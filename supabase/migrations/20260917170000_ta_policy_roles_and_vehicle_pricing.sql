...
-- 1. Role-based TA rows: expense_groups.role_ids. TA and DA group lookups are now separate
--    (TA: groups with a TA amount, DA: groups with a DA amount), matching how the admin UI lists them.
-- 2. get_monthly_expense_summary: role rows, vehicle fixed price/day, per-employee vehicle exceptions,
--    and "no vehicle" (Outstation) days pay no TA in both methods.
-- Priority when no vehicle is picked: user override -> group member -> team override -> role row -> company default.
-- When a vehicle is picked: employee's vehicle exception -> vehicle rate/price -> the person's TA above.
