CREATE TABLE IF NOT EXISTS public.ta_rate_history (
...
DROP POLICY IF EXISTS "ta_rate_history read authenticated" ON public.ta_rate_history;
CREATE POLICY "ta_rate_history read authenticated"
ON public.ta_rate_history FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "ta_rate_history admin write" ON public.ta_rate_history;
CREATE POLICY "ta_rate_history admin write"
ON public.ta_rate_history FOR ALL TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

DROP TRIGGER IF EXISTS update_ta_rate_history_updated_at ON public.ta_rate_history;
