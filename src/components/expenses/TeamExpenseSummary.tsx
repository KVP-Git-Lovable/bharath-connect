  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-between gap-2 rounded-xs border bg-card p-1 shadow-card sm:justify-center">
      ...
      {loading ? (
        ...
        const renderExpense = (exp: TeamExpense) => (
          <Card key={exp.id} className="rounded-xs shadow-card transition-shadow hover:shadow-elevated">
          ...
          <div className="space-y-6">
            <section aria-labelledby="expense-summary-heading">
              ...
              <div className={`grid grid-cols-2 gap-3 ${daApplicable ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
                <Card className="col-span-2 rounded-xs border-primary/15 bg-primary/5 shadow-card lg:col-span-1">
            ...
            <section aria-labelledby="pending-approvals-heading">
              ...
              {pendingExpenses.length === 0 ? <Card className="rounded-xs border-dashed shadow-none"><CardContent className="py-8 text-center text-sm text-muted-foreground">No pending approvals for this month.</CardContent></Card> : <div className="grid gap-3 lg:grid-cols-2">{pendingExpenses.map(renderExpense)}</div>}
            </section>
            ...
            <section aria-labelledby="team-expenses-heading">
              ...
              {summariesLoading ? <div className="flex justify-center py-6"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div> : memberSummaries.length === 0 ? <Card className="rounded-xs border-dashed shadow-none"><CardContent className="py-8 text-center text-sm text-muted-foreground">No team members found.</CardContent></Card> : <div className="grid gap-3 lg:grid-cols-2">{memberSummaries.map((u) => <Card key={u.user_id} className="rounded-xs shadow-card"><CardContent className="p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">{u.name}</p><p className="text-xs text-muted-foreground">{u.present_days} present · {u.total_km.toFixed(1)} km</p></div><span className="text-sm font-bold">₹{(daApplicable ? u.total : u.total - u.da).toFixed(0)}</span></div><div className={`grid ${daApplicable ? 'grid-cols-3' : 'grid-cols-2'} gap-2 text-xs`}><div className="rounded-xs bg-info/10 p-2 text-center text-info"><p className="text-xs">TA</p><p className="font-semibold">₹{u.ta.toFixed(0)}</p></div>{daApplicable && <div className="rounded-xs bg-success/10 p-2 text-center text-success"><p className="text-xs">DA</p><p className="font-semibold">₹{u.da.toFixed(0)}</p></div>}<div className="rounded-xs bg-accent/10 p-2 text-center text-accent"><p className="text-xs">Add</p><p className="font-semibold">₹{u.additional.toFixed(0)}</p></div></div></CardContent></Card>)}</div>}
            </section>
          </div>
        );
      })()}
